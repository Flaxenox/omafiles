...
3. **Pure Function Centralization:** Moved pure MIME/file type helpers (`extOf`, `iconFor`, `isImage`, `isVideo`, `isAudio`, `isPdf`) directly into `Utils.js`, deleting `logic/FileTypeUtils.qml`.
...
### A. Composition Root (`core/OmafilesContent.qml`)
...
### B. Main Visual Hierarchy (`core/MainLayout.qml`)
...
- Child panels (`panels/Sidebar.qml`, `panels/BackgroundPanel.qml`, `shared/PathCompletionField.qml`, `panels/SearchBar.qml`) access controllers via `controllers.*` and commands via `commandFacade.*`.

### C. Modal Dialog Layer (`core/DialogLayer.qml`)
...
### D. Active File List & Shortcuts (`panels/ActiveFileList.qml` & `logic/KeyboardShortcuts.qml`)
...
### E. File Type Pure Utilities Centralization (`Utils.js`)
- Pure functions `extOf(name)`, `iconFor(entry)`, `isImage(entry)`, `isVideo(entry)`, `isAudio(entry)`, `isPdf(entry)` added to `Utils.js`.
- Deleted obsolete `logic/FileTypeUtils.qml` and unregistered from `ControllerRegistry.qml`.
...
### F. Navigation Controller (`logic/NavigationController.qml`)
