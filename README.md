...
- Trash aggregates every active trash location, not just the one under your home folder — anything deleted from another mounted drive gets its own trash there too (standard behavior, avoids copying across devices just to delete something), and Omafiles shows all of them together in one place instead of only the home one. Each item shows its original location and deletion time (read from `.trashinfo`).
